...
async function getTimestampedDownloadDir() {
  const downloadsDir = join(homedir(), "Downloads");
  const timestamp = Date.now();
  const dirName = `visit-website-${timestamp}`;
  const dirPath = join(downloadsDir, dirName);
...
export async function toolsProvider(
  ...
  const extractLinks = (
    ...
    [...body.matchAll(/<a\s+[^>]*?href="([^"]+)"[^>]*>((?:\n|.)*?)<\/a>/g)]
      ...
      .map((x, index, { length }) => {
        ...
        const score =
          ratio *
            (100 - (x.label.length + x.link.length + (20 * index) / length)) +
    ...
    [...body.matchAll(/<img(\s+[^>]*)/g)]
      ...
      .map(([, attributes], index) => {
        ...
        return {
          index,
          alt,
          src: src?.startsWith("/") ? new URL(src, url).href : src,
          score:
            (searchTerms?.length &&
              searchTerms.reduce(
                (acc, term) =>
                  acc +
                  (alt.toLowerCase().includes(term.toLowerCase()) ? 1000 : 0),
                alt.length,
              )) ||
            alt.length,
        };
      ...
      .filter(
        ({ src }) =>
          src &&
          src.startsWith("http") &&
          src.match(/\.(svg|png|webp|gif|jpe?g)(\?.*)?$/i),
  ...
  const downloadFileTool = tool({
    name: "Download File or Website",
    description: "Download files (PDFs, documents, etc.) or convert a website to PDF. Files are saved to the Downloads folder in a timestamped directory.",
    parameters: {
      fileURLs: z
        .array(z.string().url())
        .optional()
        .describe("List of file URLs to download (PDFs, documents, etc.)."),
      websiteURL: z
        .string()
        .url()
        .optional()
        .describe("The URL of the website to download as PDF."),
    },
    implementation: async (
      ...
      try {
        const urlsToDownload = fileURLs || [];

        if (websiteURL) {
          status("Downloading website as PDF...");
          urlsToDownload.push(websiteURL);
        }

        if (urlsToDownload.length === 0) {
          warn("No URLs provided to download.");
          return "No URLs provided to download.";
        }

        status("Downloading files...");
        const downloadDir = await getTimestampedDownloadDir();
        const downloadPromises = urlsToDownload.map(
          async (url: string, i: number) => {
            if (url.startsWith(homedir())) return url;
            ...
            try {
              ...
              if (!fileResponse.ok) {
                warn(
                  `Failed to fetch file ${index}: ${fileResponse.statusText}`,
                );
                return null;
              }
              const bytes = await fileResponse.bytes();
              if (bytes.length === 0) {
                warn(`File ${index} is empty: ${url}`);
                return null;
              }
              ...
              if (error instanceof DOMException && error.name === "AbortError")
                return null;
              warn(`Error fetching file ${index}: ${error.message}`);
              return null;
            }
          },
        );
        const downloadedFiles = await Promise.all(downloadPromises);
        const successfulDownloads = downloadedFiles.filter((x) => x !== null);

        if (successfulDownloads.length === 0) {
          warn("Error downloading files");
          return urlsToDownload;
        }

        status(
          `Downloaded ${successfulDownloads.length} files to ${downloadDir}`,
        );
        ...
        console.error(error);
        warn(`Error during file download: ${error.message}`);
  ...
  const viewImagesTool = tool({
    name: "View Images",
    description:
      "Download images from a website or a list of image URLs. Files are saved to the Downloads folder in a timestamped directory.",
    parameters: {
      imageURLs: z
        .array(z.string().url())
        .optional()
        .describe(
          "List of image URLs to view that were not obtained via the Visit Website tool.",
        ),
      websiteURL: z
        .string()
        .url()
        .optional()
        .describe("The URL of the website, whose images to view."),
      maxImages: z
        .number()
        .int()
        .min(1)
        ...
        .optional()
        .describe(
          "Maximum number of images to view when websiteURL is provided.",
        ),
    },
    implementation: async (
      { imageURLs, websiteURL, maxImages },
      { status, warn, signal },
    ) => {
      try {
        maxImages =
          undefinedIfAuto(
            ctl.getPluginConfig(configSchematics).get("maxImages"),
            -1,
          ) ??
          maxImages ??
          10;
        ...
        if (websiteURL) {
          ...
          const images = extractImages(body, websiteURL, maxImages).map(
            (x) => x[1],
          );
        ...
        const downloadDir = await getTimestampedDownloadDir();
        const downloadPromises = imageURLsToDownload.map(
          async (url: string, i: number) => {
            if (url.startsWith(homedir())) return url;
        ...
        const downloadedImageMarkdowns = (
        ...
        status(
          `Downloaded ${downloadedImageMarkdowns.length} images to ${downloadDir}`,
        );
  ...
  const visitWebsiteTool = tool({
    name: "Visit Website",
    description:
      "Visit a website and return its title, headings, links, images, and text content. Images are automatically downloaded to the Downloads folder in a timestamped directory.",
    parameters: {
    ...
    implementation: async (
      { url, maxLinks, maxImages, contentLimit, findInPage: searchTerms },
      context,
    ) => {
      ...
      try {
        maxLinks =
          undefinedIfAuto(
            ctl.getPluginConfig(configSchematics).get("maxLinks"),
            -1,
          ) ??
          maxLinks ??
          40;
        ...
        contentLimit =
          undefinedIfAuto(
            ctl.getPluginConfig(configSchematics).get("contentLimit"),
            -1,
          ) ??
          contentLimit ??
          2000;
        ...
        const links =
          maxLinks && extractLinks(body, url, maxLinks, searchTerms);
        const imagesToFetch = maxImages
          ? extractImages(body, url, maxImages, searchTerms)
          : [];
        const images =
          maxImages &&
          (
            (await viewImagesTool.implementation(
              { imageURLs: imagesToFetch.map((x) => x[1]) },
              context,
            )) as string[]
          ).map(
            (markdown, index) =>
              [imagesToFetch[index][0], markdown] as [string, string],
          );
        ...
        if (searchTerms?.length && contentLimit < allContent.length) {
          ...
          const matches = searchTerms
            .map((term) =>
              new RegExp(padding + term + padding, "gi").exec(allContent),
            )
            .filter((match) => !!match)
